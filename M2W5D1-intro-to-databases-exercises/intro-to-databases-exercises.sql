...
SELECT name AS "Name", governmentform AS "Form of Gov't", headofstate AS "Head of State" FROM country WHERE continent = 'Europe';
-- 4. The name, population, surface area, and average life expectancy of all countries in Asia (51 rows)
SELECT name AS "Name", population AS "Pop.", surfacearea AS "Surface Area", lifeexpectancy AS "Life Expectancy" FROM country WHERE continent = 'Asia';
...
SELECT name, CAST((gnp / population * 1000000) AS DECIMAL(38, 2)) AS "Per Capita GNP" FROM country WHERE continent = 'Europe'
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, (2017 - indepyear) AS "Years Since Indepence" FROM country WHERE indepyear IS NOT NULL;
=======
-- 1. The name and population of all cities in Ontario, Canada (27 rows)
